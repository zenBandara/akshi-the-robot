...
try:
    while running:
        ...
        if center is not None:
            ...
            if DRAW_RADII:
                cv2.circle(frame_bgr, (FRAME_W//2, FRAME_H//2), LOCK_RADIUS_PIX, (0,255,0), 1)
                cv2.circle(frame_bgr, (FRAME_W//2, FRAME_H//2), REACQUIRE_RADIUS_PIX, (0,165,255), 1)
            ...
            cv2.circle(frame_bgr, (int(ax_f), int(ay_f)), 4, (0,255,0) if moving else (0,200,255), -1)
        else:
        ...
        cv2.putText(frame_bgr, f"SrvYaw:{yaw_deg:5.1f}  SrvPitch:{pitch_deg:5.1f}  {'MOVE' if moving else 'HOLD'}",
